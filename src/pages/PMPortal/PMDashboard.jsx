 ...
 * Nav:     Nav B — per UrbanNest_LeftNav.md
 * Header:  Standard — per UrbanNest_DesignSystem.md §8
 * Layout:  2-col shell (NavB 185px fixed + scrollable content area)
 * Reuses:  CustomDropdown, Button (from src/components/ui)
 * Session: 6 — updated May 28, 2026
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ─── Mock data ─────────────────────────────────────────────────────────────────
const MOCK_METRICS = {
  totalUnits: 24, totalProperties: 6,
  occupancyRate: 87.5, occupancyTrend: +2.3,
  revenueMTD: 18420, collectionRate: 94,
  openMaintenance: 7, overdueCount: 2,
};

const MOCK_LEASING = [
  { label: 'Active Leads',    value: 12, danger: false },
  { label: 'Applications',    value: 4,  danger: false },
  { label: 'Expiring Leases', value: 3,  danger: true  },
  { label: 'Avg Days',        value: 18, danger: false },
];

const MOCK_SNAPSHOT = [
  { label: 'Residential',    value: 14, color: '#002D5B' },
  { label: 'Commercial',     value: 5,  color: '#16A34A' },
  { label: 'Student Housing',value: 3,  color: '#D97706' },
  { label: 'Short-term',     value: 2,  color: '#7C3AED' },
];
...
// Default Unsplash images — replaced when actual property images uploaded
...
// ─── MiniSelector — pill-style selector for control bar (NOT a form field) ────
// Intentionally NOT CustomDropdown — control bar needs compact pill style
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <div onClick={() => setOpen(p => !p)}
        style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '4px 10px', background: value ? C.contextBg : C.neutral, border: '1px solid ' + (value ? C.contextBorder : C.borderMedium), borderRadius: '20px', cursor: 'pointer', transition: 'all 0.12s', whiteSpace: 'nowrap' }}
      >
        <span style={{ fontFamily: F.body, fontSize: '12px', fontWeight: value ? 600 : 400, color: value ? C.primary : C.textSecondary }}>
          {selected?.label || options[0]?.label}
        </span>
        <i className={`ti ${open ? 'ti-chevron-up' : 'ti-chevron-down'}`} style={{ fontSize: '11px', color: value ? C.primary : C.textTertiary }} />
      </div>
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, zIndex: 200, background: C.white, border: '1px solid ' + C.contextBorder, borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,45,91,0.10)', minWidth: '140px', overflow: 'hidden' }}>
          {options.map(o => (
            <div key={o.value} onClick={() => { onChange(o.value); setOpen(false); }}
              style={{ padding: '8px 12px', fontFamily: F.body, fontSize: '12px', color: o.value === value ? C.primary : C.textPrimary, background: o.value === value ? C.contextBg : C.white, fontWeight: o.value === value ? 600 : 400, cursor: 'pointer', transition: 'background 0.1s' }}
...
// ─── Metric Card — left border accent, center-aligned, 20% shorter ────────────
  ...
  return (
    <div style={{ background: C.white, border: '1px solid ' + C.border, borderRadius: '12px', padding: '12px 16px', flex: 1, minWidth: 0, borderLeft: `3px solid ${accentColor}`, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '8px', width: '100%' }}>
        ...
        {trend !== undefined && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '3px', background: trend > 0 ? C.greenBg : C.dangerBg, borderRadius: '20px', padding: '2px 7px' }}>
...
// ─── Leasing Activity Card — 2×2 grid layout ──────────────────────────────────
function LeasingActivityCard() {
  return (
    <div style={{ background: C.white, border: '1px solid ' + C.border, borderRadius: '12px', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      {/* 2×2 grid pushed down with marginTop */}
...
// ─── Property Snapshot Card — donut centered, legend stacked below ─────────────
function PropertySnapshotCard() {
  const total = MOCK_SNAPSHOT.reduce((s, i) => s + i.value, 0);
  const r = 38, circ = 2 * Math.PI * r;
  let offset = 0;
  const segments = MOCK_SNAPSHOT.map(item => {
  ...
  return (
    <div style={{ background: C.white, border: '1px solid ' + C.border, borderRadius: '12px', padding: '12px 14px', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Legend — full width, stacked below */}
...
// ─── Financial Pulse Card — Option A: dark navy, vertical stacked rectangles ──
  ...
  return (
    <div style={{ background: C.navyCard, border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
        {rows.map(row => (
          <div key={row.label}
            style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0 7px 7px 0', borderLeft: `3px solid ${row.accentColor}`, padding: '8px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
          >
            <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px' }}>
        <Button variant="secondary" fullWidth={false}
          style={{ height: '32px', fontSize: '11px', color: C.white, borderColor: 'rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.08)', width: '100%' }}
        >
          View P&L
        </Button>
        <Button variant="primary" fullWidth={false}
          style={{ height: '32px', fontSize: '11px', background: C.green, width: '100%' }}
        >
          Export
        </Button>
      </div>
...
// ─── Recent Transactions Card — Property | Type | Amount ──────────────────────
  ...
  return (
    <div style={{ background: C.white, border: '1px solid ' + C.border, borderRadius: '12px', padding: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
        ...
        <div ref={filterRef} style={{ position: 'relative' }}>
          <div onClick={() => setFilterOpen(p => !p)}
            style={{ width: '28px', height: '28px', borderRadius: '6px', border: '1px solid ' + (txFilter !== 'All' ? C.primary : C.borderMedium), background: txFilter !== 'All' ? C.contextBg : C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.12s' }}
          >
            <i className="ti ti-adjustments-horizontal" style={{ fontSize: '13px', color: txFilter !== 'All' ? C.primary : C.textSecondary }} />
          </div>
          {filterOpen && (
            <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, zIndex: 200, background: C.white, border: '1px solid ' + C.contextBorder, borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,45,91,0.10)', minWidth: '160px', overflow: 'hidden' }}>
              {TYPES.map(t => (
                <div key={t} onClick={() => { setTxFilter(t); setFilterOpen(false); }}
                  style={{ padding: '8px 12px', fontFamily: F.body, fontSize: '12px', color: t === txFilter ? C.primary : C.textPrimary, background: t === txFilter ? C.contextBg : C.white, fontWeight: t === txFilter ? 600 : 400, cursor: 'pointer', transition: 'background 0.1s' }}
                  onMouseEnter={e => { if (t !== txFilter) e.currentTarget.style.background = '#F0F5FF'; }}
                  onMouseLeave={e => { if (t !== txFilter) e.currentTarget.style.background = C.white; }}
                >
                  {t}
                </div>
              ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '8px', padding: '0 0 6px', borderBottom: '1px solid ' + C.border, marginBottom: '4px' }}>
        {['Property', 'Type', 'Amount'].map(h => (
          <span key={h} style={{ fontFamily: F.body, fontSize: '10px', fontWeight: 700, color: C.textTertiary, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</span>
        ))}
      </div>
...
function PropertyPanel({ properties, onClose }) {
  ...
  const typeBadge = type => {
    if (type === 'For Rent') return { bg: C.greenBg,    color: C.green   };
    if (type === 'For Sale') return { bg: C.contextBg,  color: C.primary };
  ...
  return (
    <div style={{ width: '240px', minWidth: '200px', background: C.white, borderLeft: '1px solid ' + C.border, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 14px 8px', borderBottom: '1px solid ' + C.border, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <div ref={filterRef} style={{ position: 'relative' }}>
              <div onClick={() => setFilterOpen(p => !p)}
                style={{ width: '24px', height: '24px', borderRadius: '5px', border: '1px solid ' + (propFilter !== 'All' ? C.primary : C.borderMedium), background: propFilter !== 'All' ? C.contextBg : C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              >
                <i className="ti ti-adjustments-horizontal" style={{ fontSize: '12px', color: propFilter !== 'All' ? C.primary : C.textSecondary }} />
              </div>
              {filterOpen && (
                <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, zIndex: 200, background: C.white, border: '1px solid ' + C.contextBorder, borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,45,91,0.10)', minWidth: '130px', overflow: 'hidden' }}>
                  {TYPES.map(t => (
                    <div key={t} onClick={() => { setPropFilter(t); setFilterOpen(false); }}
                      style={{ padding: '7px 12px', fontFamily: F.body, fontSize: '12px', color: t === propFilter ? C.primary : C.textPrimary, background: t === propFilter ? C.contextBg : C.white, fontWeight: t === propFilter ? 600 : 400, cursor: 'pointer' }}
                      onMouseEnter={e => { if (t !== propFilter) e.currentTarget.style.background = '#F0F5FF'; }}
                      onMouseLeave={e => { if (t !== propFilter) e.currentTarget.style.background = C.white; }}
                    >
                      {t}
                    </div>
                  ))}
        ...
        <p style={{ margin: 0, fontFamily: F.body, fontSize: '11px', color: C.textTertiary }}>
          {properties.length === MOCK_PROPERTIES.length ? 'All cities · All' : 'Current filter'}
        </p>
      </div>
      ...
      <div className="prop-scroll" style={{ flex: 1, overflowY: 'auto', padding: '10px' }}>
        {filtered.length === 0 && (
          <p style={{ textAlign: 'center', fontFamily: F.body, fontSize: '12px', color: C.textTertiary, marginTop: '24px' }}>No properties match this filter.</p>
        )}
        {filtered.map(prop => {
          const badge = typeBadge(prop.type);
          const isSelected = prop.id === selectedId;
          return (
            <div key={prop.id} onClick={() => setSelectedId(prop.id)}
              style={{ border: isSelected ? `1.5px solid ${C.primary}` : '1px solid ' + C.border, borderRadius: '10px', overflow: 'hidden', marginBottom: '10px', cursor: 'pointer', transition: 'border-color 0.15s', background: C.white }}
            >
              <div style={{ position: 'relative', height: '90px', overflow: 'hidden', background: C.pageBg }}>
                ...
                />
                <div style={{ position: 'absolute', top: '6px', right: '6px', background: badge.bg, color: badge.color, fontFamily: F.body, fontSize: '9px', fontWeight: 700, borderRadius: '20px', padding: '2px 7px' }}>
                  {prop.type}
                </div>
              </div>
      ...
      <div style={{ flexShrink: 0, padding: '8px 12px', borderTop: '1px solid ' + C.border }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '7px', borderRadius: '7px', border: '1px solid ' + C.borderMedium, cursor: 'pointer', transition: 'background 0.15s' }}
...
function FilterPanel({ open, onClose, filters, setFilters, moduleToggles, setModuleToggles }) {
  ...
  const handleReset = () => {
    setFilters({ propertyTypes: [], occupancyStatus: [], maintPriority: [] });
    setModuleToggles(Object.fromEntries(MODULES.map(m => [m, true])));
  };
  ...
  const ChipGroup = ({ label, filterKey, chips }) => (
    <div style={{ marginBottom: '18px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
        {chips.map(chip => {
          const active = (filters[filterKey] || []).includes(chip);
          return (
            <div key={chip} onClick={() => toggleChip(filterKey, chip)}
              style={{ padding: '4px 10px', borderRadius: '20px', border: '1px solid ' + (active ? C.primary : C.borderMedium), background: active ? C.primary : C.white, color: active ? C.white : C.textSecondary, fontFamily: F.body, fontSize: '11px', fontWeight: active ? 600 : 400, cursor: 'pointer', transition: 'all 0.12s' }}
            >
              {chip}
            </div>
          );
          ...
          <div>
            ...
            {MODULES.map(mod => (
              <div key={mod} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid ' + C.border }}>
                ...
                <div onClick={() => setModuleToggles(prev => ({ ...prev, [mod]: !prev[mod] }))}
                  style={{ width: '36px', height: '20px', borderRadius: '10px', background: moduleToggles[mod] ? C.primary : C.borderMedium, position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0 }}
                >
...
// ─── Custom Reports — RBAC-controlled list ────────────────────────────────────
const MOCK_REPORTS = [
  { id: 1, name: 'Revenue Summary',        desc: 'Total revenue collected vs expected across your portfolio', category: 'Financial', lastRun: 'May 27, 2026',  roles: ['all'] },
  { id: 2, name: 'Rent Collection Report', desc: 'Breakdown of rent received, pending, and overdue by property', category: 'Financial', lastRun: 'May 28, 2026',  roles: ['all'] },
  { id: 3, name: 'Occupancy Report',       desc: 'Current occupancy rates and vacancy trends by unit type',    category: 'Operations', lastRun: 'May 26, 2026', roles: ['all'] },
  { id: 4, name: 'P&L Statement',          desc: 'Profit and loss summary with expense breakdown MTD/YTD',     category: 'Financial', lastRun: 'May 25, 2026',  roles: ['owner', 'financial_head'] },
  { id: 5, name: 'Owner Statement',        desc: 'Owner-facing income and expense report per property',        category: 'Financial', lastRun: 'May 20, 2026',  roles: ['owner'] },
  { id: 6, name: 'Maintenance Cost Report',desc: 'Vendor invoices, work order costs, and spend trends',        category: 'Operations', lastRun: 'May 22, 2026', roles: ['owner', 'property_mgr'] },
  { id: 7, name: 'Lease Expiry Report',    desc: 'Leases expiring in 30, 60, and 90 days with renewal status', category: 'Leasing',   lastRun: 'May 28, 2026',  roles: ['all'] },
  { id: 8, name: 'Tax Export',             desc: 'Annual income and expense export formatted for tax filing',  category: 'Financial', lastRun: 'Apr 15, 2026',  roles: ['owner'] },
  { id: 9, name: 'Custom Analytics',       desc: 'Build and save custom report views with your own filters',   category: 'Analytics', lastRun: 'May 10, 2026',  roles: ['owner'] },
];

const REPORT_CATEGORY_COLORS = {
  Financial:  { bg: '#EEF2FF', color: '#002D5B' },
  Operations: { bg: '#DCFCE7', color: '#166534' },
  Leasing:    { bg: '#FEF3C7', color: '#92400E' },
  Analytics:  { bg: '#F3E8FF', color: '#6B21A8' },
};
...
function CustomReportsModal({ open, onClose }) {
  ...
  return (
    <>
      ...
      {/* Centered modal — 50% of viewport width */}
      <div style={{
        position: 'fixed',
        top: '50%', left: '50%',
        ...
        <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid ' + C.border, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
            <div>
              <h2 style={{ margin: '0 0 3px', fontFamily: F.headline, fontSize: 'clamp(16px,1.6vw,20px)', fontWeight: 700, color: C.textPrimary }}>
                Custom Reports
              </h2>
              <p style={{ margin: 0, fontFamily: F.body, fontSize: '13px', color: C.textSecondary }}>
                Select a report to view or download. Access is based on your role.
              </p>
            </div>
            {/* Download + Print + Close */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '7px', border: '1px solid ' + C.borderMedium, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 0.12s' }}
                onMouseEnter={e => e.currentTarget.style.background = C.pageBg}
                onMouseLeave={e => e.currentTarget.style.background = C.white}
                title="Download"
              >
                <i className="ti ti-download" style={{ fontSize: '15px', color: C.textSecondary }} />
              </div>
              <div style={{ width: '32px', height: '32px', borderRadius: '7px', border: '1px solid ' + C.borderMedium, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 0.12s' }}
                onMouseEnter={e => e.currentTarget.style.background = C.pageBg}
                onMouseLeave={e => e.currentTarget.style.background = C.white}
                title="Print"
              >
                <i className="ti ti-printer" style={{ fontSize: '15px', color: C.textSecondary }} />
              </div>
              <div onClick={onClose} style={{ width: '32px', height: '32px', borderRadius: '7px', background: C.pageBg, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 0.12s' }}
            ...
            {categories.map(cat => {
              ...
              return (
                <div key={cat} onClick={() => setActiveCategory(cat)}
                  style={{ padding: '4px 12px', borderRadius: '20px', border: '1px solid ' + (isActive ? C.primary : C.borderMedium), background: isActive ? C.primary : C.white, color: isActive ? C.white : C.textSecondary, fontFamily: F.body, fontSize: '11px', fontWeight: isActive ? 700 : 400, cursor: 'pointer', transition: 'all 0.12s' }}
                >
                  {cat}
                </div>
              );
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 22px 16px', scrollbarWidth: 'thin', scrollbarColor: C.borderMedium + ' transparent' }}>
          {filtered.map((report, idx) => {
            const catStyle = REPORT_CATEGORY_COLORS[report.category] || { bg: C.pageBg, color: C.textSecondary };
            return (
              <div key={report.id}
                style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 0', borderBottom: idx < filtered.length - 1 ? '1px solid ' + C.border : 'none' }}
              >
                {/* Icon */}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                    ...
                    <span style={{ background: catStyle.bg, color: catStyle.color, fontFamily: F.body, fontSize: '9px', fontWeight: 700, borderRadius: '4px', padding: '2px 6px', letterSpacing: '0.04em' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                  <button style={{ height: '30px', padding: '0 12px', background: C.white, border: '1px solid ' + C.borderMedium, borderRadius: '6px', fontFamily: F.body, fontSize: '11px', fontWeight: 600, color: C.textSecondary, cursor: 'pointer', transition: 'all 0.12s' }}
                    onMouseEnter={e => { e.currentTarget.style.background = C.pageBg; e.currentTarget.style.borderColor = C.primary; e.currentTarget.style.color = C.primary; }}
                    onMouseLeave={e => { e.currentTarget.style.background = C.white; e.currentTarget.style.borderColor = C.borderMedium; e.currentTarget.style.color = C.textSecondary; }}
                  >
                    <i className="ti ti-download" style={{ fontSize: '11px', marginRight: '4px' }} />
                    Export
                  </button>
                  <button style={{ height: '30px', padding: '0 12px', background: C.primary, border: 'none', borderRadius: '6px', fontFamily: F.body, fontSize: '11px', fontWeight: 700, color: C.white, cursor: 'pointer', transition: 'background 0.12s' }}
        ...
        <div style={{ flexShrink: 0, padding: '12px 22px', borderTop: '1px solid ' + C.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: F.body, fontSize: '11px', color: C.textTertiary }}>
            Showing {filtered.length} of {MOCK_REPORTS.length} reports based on your role
          </span>
          <button onClick={onClose} style={{ height: '32px', padding: '0 16px', background: C.white, border: '1px solid ' + C.borderMedium, borderRadius: '7px', fontFamily: F.body, fontSize: '12px', fontWeight: 600, color: C.textSecondary, cursor: 'pointer' }}>
            Close
          </button>
        </div>
      </div>
...
export default function PMDashboard() {
  ...
  // Filter properties by current city/property selection
    ...
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: C.pageBg, fontFamily: F.body }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
          ...
          {/* Body: dashboard content + property panel */}
          <div style={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>

            {/* ── Dashboard content — only scrollable column ── */}
            <div className="dash-scroll" style={{ flex: 1, overflowY: 'auto', padding: 'clamp(16px,2vw,24px) clamp(20px,3vw,36px) 32px', minWidth: 0 }}>
              ...
              {/* ── Control Bar ── */}
              <div style={{ background: C.white, border: '1px solid ' + C.border, borderRadius: '12px', marginBottom: '10px', overflow: 'hidden', animation: 'fadein 0.3s ease 0.05s both' }}>
                {/* Row 1: Viewing + mini selectors */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: 'clamp(10px,1.5vw,14px) clamp(14px,2vw,18px) 10px', flexWrap: 'wrap' }}>
                  ...
                  {/* City — MiniSelector (pill style, same size as locked badge) */}
                  <MiniSelector options={CITY_OPTIONS} value={selectedCity} onChange={v => { setSelectedCity(v); setSelectedProp(''); }} />
                  <i className="ti ti-chevron-right" style={{ fontSize: '11px', color: C.textTertiary }} />
                  {/* Property — MiniSelector */}
                  <MiniSelector options={PROPERTY_OPTIONS} value={selectedProp} onChange={setSelectedProp} />

                  <div style={{ flex: 1 }} />

                  {/* Property panel toggle — icon only with count badge */}
                  <div onClick={() => setPropPanelOpen(p => !p)}
                    style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 9px', background: propPanelOpen ? C.contextBg : C.neutral, border: '1px solid ' + (propPanelOpen ? C.primary : C.borderMedium), borderRadius: '7px', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0 }}
                    onMouseEnter={e => { if (!propPanelOpen) { e.currentTarget.style.background = '#F0F5FF'; e.currentTarget.style.borderColor = C.contextBorder; } }}
                    onMouseLeave={e => { if (!propPanelOpen) { e.currentTarget.style.background = C.neutral; e.currentTarget.style.borderColor = C.borderMedium; } }}
                  ...
                  >
                    ...
                    <span style={{ background: propPanelOpen ? C.primary : C.borderMedium, color: propPanelOpen ? C.white : C.textSecondary, fontFamily: F.body, fontSize: '9px', fontWeight: 700, borderRadius: '99px', padding: '1px 5px', minWidth: '16px', textAlign: 'center' }}>
                      {filteredProperties.length}
                    </span>
                  </div>

                  {/* Custom Reports icon — icon only */}
                  <div onClick={() => setReportsOpen(true)}
                    style={{ width: '32px', height: '32px', borderRadius: '7px', border: '1px solid ' + C.borderMedium, background: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0 }}
                    onMouseEnter={e => { e.currentTarget.style.background = C.contextBg; e.currentTarget.style.borderColor = C.contextBorder; }}
                    onMouseLeave={e => { e.currentTarget.style.background = C.white; e.currentTarget.style.borderColor = C.borderMedium; }}
                ...
                <div style={{ display: 'flex', padding: '0 clamp(14px,2vw,18px)', borderTop: '1px solid ' + C.border }}>
                  {DATE_TABS.map(tab => {
                    const isActive = activeDate === tab;
                    return (
                      <button key={tab} onClick={() => setActiveDate(tab)}
                        style={{ padding: '8px clamp(8px,1.2vw,14px)', background: 'none', border: 'none', borderBottom: isActive ? `2px solid ${C.primary}` : '2px solid transparent', fontFamily: F.body, fontSize: '12px', fontWeight: isActive ? 700 : 400, color: isActive ? C.primary : C.textTertiary, cursor: 'pointer', transition: 'all 0.12s', marginBottom: '-1px' }}
                      >
                        {tab}
                      </button>
                    );
              ...
              {/* ── Metric Cards — center aligned, 20% shorter ── */}
              ...
              {/* ── Row 1: Leasing Activity | Property Snapshot | Financial Pulse ── */}
              ...
              {/* ── Row 2: Recent Activity | Recent Transactions ── */}
            ...
            {propPanelOpen && (
              <PropertyPanel
                ...
                onClose={() => setPropPanelOpen(false)}
              />
            )}
          </div>
        </div>
      ...
      <FilterPanel
        ...
        setModuleToggles={setModuleToggles}
      />
    </>
  );
}
