...
// ─── Estate image + testimonials (same as LoginPage left panel) ────────────────
...
// ─── Persona catalogue ─────────────────────────────────────────────────────────
const ALL_PERSONAS = [
  { key: 'RENTER',          label: 'Renter',                  sub: 'Main Account',         icon: 'ti-home',               color: C.primary,  route: '/coming-soon' },
  { key: 'TENANT',          label: 'Tenant',                  sub: 'Active Tenancy',        icon: 'ti-users',              color: '#1D4ED8',  route: '/coming-soon' },
  { key: 'LANDLORD',        label: 'Landlord',                sub: 'Property Owner',        icon: 'ti-building',           color: '#064E3B',  route: '/coming-soon' },
  { key: 'INDEPENDENT_PM',  label: 'Property Manager',        sub: 'Independent PM',        icon: 'ti-briefcase',          color: '#B45309',  route: '/pm-registration' },
  { key: 'REAL_ESTATE_AGENT', label: 'Real Estate Agent',     sub: 'Agent Portal',          icon: 'ti-id-badge',           color: '#7C3AED',  route: '/coming-soon' },
  { key: 'PMS',             label: 'PMS Company',             sub: 'Organisational',        icon: 'ti-building-skyscraper',color: '#BE123C',  route: '/onboarding' },
];

// ─── Testimonial carousel (mirrors LoginPage) ──────────────────────────────────
function TestimonialCarousel() {
  ...
  return (
    <div style={{
      ...
      <p style={{
        fontFamily: F.headline, fontSize: 14, fontWeight: 400,
        color: C.white, lineHeight: 1.7, margin: '0 0 20px',
        fontStyle: 'italic',
      }}>
        {t.quote}
      </p>
...
function PersonaCard({ persona, isActive, onClick }) {
  ...
  const bg      = active || hovered ? '#F0F4F8' : C.white;
  const border  = active
    ? `2px solid ${C.primary}`
    : hovered
      ? `2px solid ${C.borderMedium}`
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:      active
          ? `0 4px 16px rgba(0,45,91,0.12)`
          : hovered ? '0 4px 12px rgba(0,0,0,0.06)' : '0 1px 3px rgba(0,0,0,0.04)',
        textAlign:      'center',
        minHeight:      '120px',
      }}
    >
      ...
      <div style={{
        ...
        height:         '42px',
        borderRadius:   '50%',
        background:     active ? C.primary : hovered ? `${color}18` : C.neutral,
        display:        'flex',
        alignItems:     'center',
        justifyContent: 'center',
    ...
    >
      ...
      <div style={{
        ...
        <i
          ...
          style={{ fontSize: '15px', color: hovered ? C.primary : C.textTertiary }}
          aria-hidden="true"
        />
      </div>
...
export default function PersonaSelectPage() {
  ...
  const inactivePersonas = ALL_PERSONAS.filter(p => !activeKeys.includes(p.key));
  const visiblePersonas  = showAll ? ALL_PERSONAS : activePersonas;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* ── Modal — exact same dimensions as LoginPage ── */}
        <div style={{
          width:        '75vw',
          maxHeight:    '90vh',
          borderRadius: '20px',
          ...
          display:      'flex',
          animation:    'un-pagein 0.35s ease both',
        }}>

          {/* ── LEFT — hero + testimonial (mirrors LoginPage exactly) ── */}
          <div style={{
            position:   'relative',
            width:      '52%',
            minHeight:  '100%',
            ...
            {/* Large background R — subtle brand mark */}
            ...
            <div>
              ...
              {error && (
                <div style={{
                  ...
                  fontFamily: F.body, fontSize: '12px', color: '#DC2626',
                }}>
                  {error}
                </div>
              )}
              ...
              <div style={{
                ...
                gap:                 '14px',
                marginBottom:        showAll && inactivePersonas.length ? '0' : '0',
              }}>
                ...
                {!showAll && (
                  <AddPersonaCard onClick={() => setShowAll(true)} />
                )}
              </div>

              {/* Available to register (expanded) */}
              {showAll && inactivePersonas.length > 0 && (
                <>
                  <p style={{
                    margin:        '24px 0 10px',
                  ...
                  <div style={{
                    display: 'grid',
                    gridTemplateColumns: `repeat(${Math.min(inactivePersonas.length, 3)}, 1fr)`,
                    gap: '14px',
                  }}>
                ...
                <div style={{ marginTop: '16px', textAlign: 'center' }}>
                  <button
                    ...
                    style={{
                      ...
                      color: C.textSecondary, display: 'inline-flex', alignItems: 'center', gap: '4px',
                    }}
                  >
