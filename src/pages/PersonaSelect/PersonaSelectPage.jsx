...
// ─── Persona catalogue ─────────────────────────────────────────────────────────
// route = registration route (used only when persona is NOT yet active)
// portal = destination when persona IS already active
const ALL_PERSONAS = [
  ...
  { key: 'INDEPENDENT_PM',    label: 'Property Manager',    sub: 'Independent PM',    icon: 'ti-briefcase',           color: '#B45309',  route: '/pm-registration', portal: '/pm-portal/dashboard/my-dashboard' },
  ...
  { key: 'PMS',               label: 'PMS Company',         sub: 'Organisational',    icon: 'ti-building-skyscraper', color: '#BE123C',  route: '/onboarding',   portal: '/coming-soon' },
];
...
function TestimonialCarousel() {
  ...
  return (
    <div style={{
      position: 'absolute', bottom: 48, left: 36, right: 36,
      ...
      <p style={{ fontFamily: F.headline, fontSize: 14, fontWeight: 400, color: C.white, lineHeight: 1.7, margin: '0 0 20px', fontStyle: 'italic' }}>
        {t.quote}
      </p>
...
function PersonaCard({ persona, isActive, isInactive, onClick }) {
  ...
  const border = isActive
    ...
    : isInactive
      ? `2px solid #F59E0B`
      : hovered ? `2px solid ${C.borderMedium}` : `1.5px solid ${C.border}`;
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '21px 10px 16px', borderRadius: '11px', border, background: bg,
        cursor: 'pointer', transition: 'all 0.18s ease',
        transform: hovered && !isActive && !isInactive ? 'translateY(-1px)' : 'none',
        boxShadow: isActive ? '0 4px 16px rgba(0,45,91,0.12)' : isInactive ? '0 4px 16px rgba(245,158,11,0.10)' : hovered ? '0 4px 12px rgba(0,0,0,0.06)' : '0 1px 3px rgba(0,0,0,0.04)',
    ...
    >
      ...
      {isActive && (
        <div style={{
          ...
          background: C.primary, color: C.white, fontFamily: F.body, fontSize: '9px',
      ...
      {isInactive && (
        <div style={{
          position: 'absolute', top: '-11px', left: '50%', transform: 'translateX(-50%)',
          background: '#F59E0B', color: C.white, fontFamily: F.body, fontSize: '9px',
          fontWeight: 700, letterSpacing: '0.1em', padding: '3px 10px',
          borderRadius: '99px', textTransform: 'uppercase', whiteSpace: 'nowrap',
        }}>
          Inactive
      ...
      <div style={{
        ...
        background: isActive ? C.primary : isInactive ? '#FEF3C7' : hovered ? `${color}18` : C.neutral,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  return (
    <div
      ...
      style={{
        ...
        justifyContent: 'center', gap: '8px', padding: '21px 10px 16px',
        borderRadius: '11px', border: `1.5px dashed ${hovered ? C.primary : C.borderMedium}`,
        background: hovered ? '#F8FAFC' : 'transparent', cursor: 'pointer',
        transition: 'all 0.18s ease', textAlign: 'center', minHeight: '120px',
      }}
    >
      <div style={{
        width: '42px', height: '42px', borderRadius: '50%',
        border: `1.5px dashed ${hovered ? C.primary : C.borderMedium}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
...
export default function PersonaSelectPage() {
  ...
  useEffect(() => {
    const fetchMe = async () => {
      ...
      try {
        ...
        // Derive active persona keys — handle both response shapes
        let keys = [];
        if (Array.isArray(data.active_personas)) {
          keys = data.active_personas;
        } else if (Array.isArray(data.personas)) {
          keys = data.personas.filter(p => p.is_active).map(p => p.persona);
        } else {
          keys = ['RENTER'];
        }
        ...
        // Derive inactive persona keys
        const inactiveKeys = Array.isArray(data.inactive_personas) ? data.inactive_personas : [];
  ...
  const handleReactivateConfirm = () => {
    ...
    if (pendingReactivateKey === 'INDEPENDENT_PM') {
      navigate('/pm-portal/profile/persona?mode=edit');
    }
  ...
  // Active + inactive personas shown by default; unregistered only shown when expanded
  ...
  const activePersonas      = ALL_PERSONAS.filter(p => activeKeys.includes(p.key));
  const inactivePersonas    = ALL_PERSONAS.filter(p => !activeKeys.includes(p.key) && !inactiveKeys.includes(p.key));
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Modal */}
        <div style={{
          width: '75vw', maxHeight: '90vh', borderRadius: '20px',
          overflow: 'hidden',
          boxShadow: '0 32px 80px rgba(0,0,0,0.18), 0 2px 8px rgba(0,0,0,0.08)',
          display: 'flex', animation: 'un-pagein 0.35s ease both',
        }}>
          ...
          <div style={{ position: 'relative', width: '52%', minHeight: '100%', flexShrink: 0, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', top: 20, left: 16, fontFamily: F.headline, fontSize: 180, fontWeight: 700, color: 'rgba(255,255,255,0.07)', lineHeight: 1, userSelect: 'none', pointerEvents: 'none' }}>
              R
            </div>
            <TestimonialCarousel />
          </div>
          ...
          <div style={{
            flex: 1, background: C.white, display: 'flex', flexDirection: 'column',
            justifyContent: 'space-between', padding: '52px 60px 36px 56px', overflowY: 'auto',
          }}>
            <div>
              <h1 style={{ fontFamily: F.headline, fontSize: '39px', fontWeight: 700, color: C.primary, lineHeight: 1.15, margin: '0 0 10px', whiteSpace: 'nowrap' }}>
                Persona Selection
              </h1>
              <p style={{ fontFamily: F.body, fontSize: '13px', color: C.textSecondary, margin: '0 0 36px', lineHeight: 1.6, maxWidth: '260px' }}>
                Select a profile below to continue to your personalised dashboard and portal.
              </p>

              {error && (
                <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', padding: '8px 12px', marginBottom: '16px', fontFamily: F.body, fontSize: '12px', color: '#DC2626' }}>
                  {error}
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '14px' }}>
                {visiblePersonas.map(persona => (
                  <PersonaCard
                    key={persona.key}
                    persona={persona}
                    isActive={activeKeys.includes(persona.key)}
                    isInactive={inactiveKeys.includes(persona.key)}
              ...
              {/* Inactive personas (expanded) */}
              {showAll && unregisteredPersonas.length > 0 && (
                <>
                  <p style={{ margin: '24px 0 10px', fontFamily: F.body, fontSize: '10px', fontWeight: 700, color: C.textTertiary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            ...
            <div style={{ marginTop: '32px', paddingTop: '20px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <p style={{ margin: 0, fontFamily: F.body, fontSize: '8px', fontWeight: 600, color: C.textTertiary, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                Logged in as{' '}
                <span style={{ color: C.textSecondary }}>{displayEmail.toUpperCase()}</span>
              </p>
              <button
                onClick={() => {
                  localStorage.removeItem('access_token');
                ...
                style={{
                  height: '36px', padding: '0 20px', background: C.primary, color: C.white,
                  border: 'none', borderRadius: '8px', fontFamily: F.body, fontSize: '12px',
                  fontWeight: 700, cursor: 'pointer', letterSpacing: '0.04em',
                  textTransform: 'uppercase', transition: 'background 0.15s ease', flexShrink: 0,
                }}
      ...
      {showReactivateModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: C.white, borderRadius: '14px', padding: '28px 32px', maxWidth: '400px', width: '90%', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', animation: 'un-pagein 0.2s ease both' }}>
            ...
            <h2 style={{ margin: '0 0 8px', fontFamily: F.headline, fontSize: '18px', fontWeight: 700, color: '#0F172A' }}>
              Reactivate Profile?
            </h2>
            <p style={{ margin: '0 0 24px', fontFamily: F.body, fontSize: '13px', color: '#64748B', lineHeight: 1.6 }}>
              Your Independent PM profile is currently inactive. Would you like to reactivate it? You'll need to review and resubmit your details.
            </p>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={handleReactivateCancel}
                style={{ flex: 1, height: '42px', background: C.white, border: '1.5px solid #CBD5E1', borderRadius: '9px', fontFamily: F.body, fontSize: '13px', fontWeight: 600, color: '#64748B', cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button onClick={handleReactivateConfirm}
                style={{ flex: 1, height: '42px', background: '#002D5B', border: 'none', borderRadius: '9px', fontFamily: F.body, fontSize: '13px', fontWeight: 700, color: C.white, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                onMouseEnter={e => e.currentTarget.style.background = '#003d7a'}
                onMouseLeave={e => e.currentTarget.style.background = '#002D5B'}
              >
